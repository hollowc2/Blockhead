import type { BuildingComponent, BuildingDesign } from "./schema.js";
...
export interface BlueprintOperation { id: string; x: number; y: number; z: number; material: string; phase: BuildPhase; replaceExisting: boolean; structural: boolean; }
export interface Blueprint { origin: { x: number; y: number; z: number; dimension: string }; operations: BlueprintOperation[]; estimates: { blocks: number; materials: Record<string, number> }; footprint: { width: number; depth: number; height: number }; }
...
export function compileBuildingDesign(design: BuildingDesign, origin: { x: number; y: number; z: number; dimension: string }, limits: BuildingLimits = DEFAULT_BUILDING_LIMITS): Blueprint {
  ...
  return { origin, operations, estimates: { blocks: operations.length, materials }, footprint: { width: maxX * 2 + 1, depth: maxZ * 2 + 1, height: maxY + 1 } };
}
