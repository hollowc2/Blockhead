...
export class TerrainProjectRunner {
  ...
  async run(projectPlan: FrozenTerrainPlan, options: ExcavationRunOptions): Promise<SkillResult<ExcavationSliceData>> {
    if (projectPlan.specification.kind !== "excavate") return { ok: false, status: "failed", errorCode: "INVALID_RESOURCE", message: "terrain runner received a non-excavation project", retryable: false };
    return runExcavationSlice(this.bot, projectPlan, { ...this.options, ...options });
  }
}
