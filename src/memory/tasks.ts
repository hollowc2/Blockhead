...
function toTask(row: TaskRow): Task {
  return {
    ...
    ...(row.resume_state_json ? { resumeState: parseJson(row.resume_state_json, {}) } : {}),
    ...(row.parent_task_id === null ? {} : { parentTaskId: row.parent_task_id }),
    ...(row.interrupted_task_id === null ? {} : { interruptedTaskId: row.interrupted_task_id }),
    ...(row.last_error === null ? {} : { lastError: row.last_error }),
    ...(row.started_at === null ? {} : { startedAt: row.started_at }),
    ...(row.completed_at === null ? {} : { completedAt: row.completed_at }),
    ...(row.pause_sequence === null ? {} : { pauseSequence: row.pause_sequence }),
    ...(row.work_key === null ? {} : { workKey: row.work_key }),
    ...(row.phase === null ? {} : { phase: row.phase }),
    ...(row.progress_fingerprint === null ? {} : { progressFingerprint: row.progress_fingerprint }),
    ...(row.last_progress_at === null ? {} : { lastProgressAt: row.last_progress_at }),
    ...(row.attempts === null || row.attempts === 0 ? {} : { attempts: row.attempts }),
    ...(row.project_id === null ? {} : { projectId: row.project_id }),
    ...(row.project_phase_id === null ? {} : { projectPhaseId: row.project_phase_id }),
    ...(row.execution_policy === "resumable" || row.execution_policy === "terminal" ? { executionPolicy: row.execution_policy } : {}),
  };
}
